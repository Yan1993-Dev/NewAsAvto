<li class="accordion">
                        <button class="accordion__control" aria-expanded="false"></button>
                        <span class="accordion__title">
                            <ul class="equipment_title_two">
                                <li class="equipment_item accordion__icon"><svg max-width="19" height="11" viewBox="0 0 19 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M16.5951 0L9.71873 6.79892L2.84234 0L0.72998 2.09312L9.71873 11L18.7075 2.09312L16.5951 0Z" fill="black" />
                                    </svg></li>
                                <div class="compl">
                                    <li class="equipment_item_adapt">Комплектация</li>
                                    <li class="equipment_item_model">1.6T 7DCT 4WD 186 л.с. FLAGSHIP</li>
                                </div>
                                <div class="compl">
                                    <li class="equipment_item_price_adapt">Цена</li>
                                    <li class="equipment_item_old_price">4 249 900 руб.</li>
                                </div>
                                <div class="compl">
                                    <li class="equipment_item_new_adapt">Финальная цена</li>
                                    <li class="equipment_item_new_price">3 449 900 руб.</li>
                                </div>
                                <li class="equipment_item_credit_price">в кредит за 11 990 руб/мес или меньше!</li>
                                <li class="credit__item"><a class="equipment_item_credit_btn btn" href="#calculator">Рассчитать кредит</a></li>
                            </ul>
                        </span>
                        <div class="accordion__content" aria-hidden="true">
                        <p class="save_title_additionally">В дополнение к комплектации 1.6T 7DCT 4WD 186 л.с. LUXURY:</p>
                            <h3 class="save_title">Дизайн</h3>
                            <ul class="save">
                                <li class="save_item">19-дюймовые алюминиевые литые диски</li>
                                <li class="save_item">Панорамная крыша с люком</li>
                            </ul>
                            <h3 class="design_title">Безопасность</h3>
                            <ul class="design">
                                <li class="design_item">Адаптивный круиз-контроль (АСС)</li>
                                <li class="design_item">Система предотвращения столкновений (FCW+AEB)</li>
                                <li class="design_item">Контроль дистанции спереди (FDM)</li>
                                <li class="design_item">Система удержания в полосе (LKA)</li>
                                <li class="design_item">Система помощи в пробках (TJA+ICA)</li>
                                <li class="design_item">Предупреждение о покидании полосы (LDW)</li>
                                <li class="design_item">Система авто переключения ближний / дальний свет (IHC)</li>
                                <li class="design_item">Система распознавания знаков (TSR)</li>
                            </ul>
                            <h3 class="comfort_title">Комфорт</h3>
                            <ul class="comfort">
                                <li class="comfort_item">Атмосферная подсветка интерьера (многоцветная)</li>
                                <li class="comfort_item">Кожаная отделка сидений (Premium)</li>
                                <li class="comfort_item">Вентиляция передних сидений</li>
                                <li class="comfort_item">Зеркало заднего вида с автозатемнением</li>
                            </ul>
                            <h3 class="comfort_title">Технологии и мультимедиа</h3>
                            <ul class="comfort">
                                <li class="comfort_item">Аудио-система премиум-класса Sony (8 динамиков)</li>
                            </ul>
                        </div>
                    </li>