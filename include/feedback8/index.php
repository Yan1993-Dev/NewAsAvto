<?
require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/header.php");
$APPLICATION->SetTitle("Крупнейший официальный дилер китайских авто в Самаре, Тольятти и Саратове");
$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH . "/css/feedback.css");
?> <nav class="nav">
	<ul class="navigation">
		<li class="nav__item"><a href="/novye-avto/index.php" class="nav__item__link">Новые авто</a>
			<ul class="nav__nav">
				<li class="nav__nav__item"><a href="/geely/index.php" class="nav__nav__link">GEELY</a></li>
				<li class="nav__nav__item"><a href="/haval/index.php" class="nav__nav__link">HAVAL</a></li>
				<li class="nav__nav__item"><a href="/chery/index.php" class="nav__nav__link">CHERY</a></li>
				<li class="nav__nav__item"><a href="/exeed/index.php" class="nav__nav__link">EXEED</a></li>
				<li class="nav__nav__item"><a href="/changan/index.php" class="nav__nav__link">CHANGAN</a></li>
				<li class="nav__nav__item"><a href="/faw/index.php" class="nav__nav__link">FAW</a></li>
				<li class="nav__nav__item"><a href="/jac/index.php" class="nav__nav__link">JAC</a></li>
				<li class="nav__nav__item"><a href="/gwm/index.php" class="nav__nav__link">GWM Pickup</a></li>
				<li class="nav__nav__item"><a href="/novye-avto/index.php" class="nav__nav__link">Все автомобили</a></li>
			</ul>
		</li>
		<li class="nav__item"><a href="/servis/index.php" class="nav__item__link">Сервис</a></li>
		<li class="nav__item"><a href="/aktsii/index.php" class="nav__item__link">Акции</a></li>
		<li class="nav__item"><a href="/kredit/index.php" class="nav__item__link">Кредит</a></li>
		<li class="nav__item"><a href="/treyd-in/index.php" class="nav__item__link">Трейд-ин</a></li>
		<li class="nav__item"><a href="/novosti/index.php" class="nav__item__link">Новости</a></li>
		<li class="nav__item"><a href="/otzyvy/index.php" class="nav__item__link">Отзывы</a></li>
		<li class="nav__item"><a href="/o-nas/index.php" class="nav__item__link">О нас</a></li>
		<li class="nav__item"><a href="/kontakty/index.php" class="nav__item__link">Контакты</a></li>
	</ul>
</nav>
<main>
	<div class="background__poster">
		<div class="poster__container">
			<div class="title__poster">
				<div class="colors__poster">
					<div class="color__poster">
						<p class="title__poster__text">
							Вы сделали правильный выбор, если обратились в «АсАвто».
						</p>
						<p class="title__desc__poster">
							Убедитесь в этом сами - посмотрите отзывы наших клиентов.
						</p>
					</div>
				</div>
			</div>
			<ul class="offer">
				<li class="offer__item">Отзывы реальных клиентов</li>
				<li class="offer__item">Только честные отзывы</li>
			</ul>
		</div>
	</div>
	<div class="container_site">
		<div class="title__poster__contain">
			<h1 class="text__poster" id="calculator">Отзывы о нас</h1>
		</div>
		<div class="category">
			<ul class="category__list">
            <li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/diwkavy8fkmi0ubw717qg1yps4qmkul4.jpg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							12.02.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
						<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 7 Pro
					</p>
					<p class="category__desc">
						Хорошее обслуживание, отзывчивый персонал. Менеджер Иван молодец
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/zcov9gbcgvu8prarmymgjcb4pbg0lur5.jpg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							10.02.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
						<span class="auto_desc_under">Автомобиль:</span> EXEED TXL
					</p>
					<p class="category__desc">
						Менеджеры настоящие профессионалы своего дела. Буду советовать автосалон АсАвто всем знакомым
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/z2u4wdffzfhg4c0lwrphcbde3738logo.jpeg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							07.02.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 8 Pro
					</p>
					<p class="category__desc">
						Всё отлично. Дали скидку на автомобиль. Оценка моего автомобиля устроила, цену не завышали. Пришлось подождать одобрение кредита, порадовало что платёж по кредиту оказался меньше чем я ожидал. Спасибо АсАвто!
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/508u95pdir4mrvl4pu5hlfqoufnx5nxh.jpg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							3.02.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 4
					</p>
					<p class="category__desc">
						Доброжелательное быстрое обслуживание. Сотрудники проявили участие в каждом вопросе. очень довольны. Спасибо Александру и Ольге
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/neombq6pboor47ng025j7d7wby7ma4nw.jpg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							31.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 8 Pro
					</p>
					<p class="category__desc">
						Всё устроило. Очень профессионально подобрали авто под наши предпочтения
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/e67n89zsqmlpyr17xp53n3lvhikivoqo.jpeg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							30.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 4
					</p>
					<p class="category__desc">
						Обслуживание произвели на высоком уровне. Менеджер на протяжении всего периода взаимодействия, активно выходил на связь, консультировал по индивидуальным вопросам. Спасибо Даниилу
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/14r3t82skmwa2nl505rfi0cj83cdex1u.jpeg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							28.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 7 Pro
					</p>
					<p class="category__desc">
						Купили Chery Tiggo 7PRO на Южном шоссе, 12А. Выражаю огромную благодарность менеджеру Александру и руководителю отдела продаж Екатерине. Помогли подобрать автомобиль
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/gnwtd97fiva01oc1fyzvgz3h4kga6gtq.jpeg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							26.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> CHERY Tiggo 8 NEW
					</p>
					<p class="category__desc">
						Выражаю благодарность автосалону АсАвто и менеджеру Роману в частности за профессионализм в работе с клиентами. Свой прежний авто выгодно сдал по программе трейд ин, да еще и скидку получил. Ждите меня за следующей покупкой авто
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/jpr0o9ppltr9mvf2lf0wz8tb3819jz65.jpg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							26.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 7 PRO
					</p>
					<p class="category__desc">
						Понравилось быстрое обслуживание и не навязчивый сервис. Компетентный менеджер Виталий ответил на все вопросы. Выбор машины доставил удовольствие.
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/slamaoeau0j7pdvpdgvvtpk484o7apf2.jpeg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							24.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 4
					</p>
					<p class="category__desc">
						Обслуживание понравилось. Менеджер корректен, опрятен, профессионален. Кредитный специалист обстоятельна, ответственна. Сотрудники салона были вежливы и приятны в общении
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/ot1qssegwdxv1u6n26al29e2v5sasoub.jpg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							24.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 8
					</p>
					<p class="category__desc">
						Очень приятное и внимательное обслуживание. Владимир показал себя настоящим менеджером отдела продаж. Спасибо большое за профессионализм
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/8nbuj0z81k26xeb9i53ypi4alse1nhqu.jpg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							24.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Geely Tugella
					</p>
					<p class="category__desc">
						Всё отлично. Полностью проконсультировали по комплектациям и доп. оборудованию. Хорошо объяснили соотношение цены и комплектации автомобиля. Провели тест-драйв с подробными разъяснениями и показали интересные функции авто. Менеджер на протяжении всей сделки не выходил из поля зрения. Оперативно сделали оценку и приняли мой автомобиль по trade-in
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/g2edzgj2m8mvfrjez8jwl646wk8zlzx1.jpeg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							23.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 8
					</p>
					<p class="category__desc">
						Покупкой доволен. Спасибо менеджеру с выбором автомобиля
					</p>
				</li>
				<li class="category__item">
					<div class="image__wrapper">
						<img alt="feedback" src="/local/templates/NewAsAvto/static/images/uj6r96kdwew2tgdqraazahc4e7zqzu40.jpeg" class="category__image minimized">
					</div>
					<div class="date__info">
						<p class="date__info-item">
							23.01.2022
						</p>
					</div>
					<div class="auto__desc">
						<span class="auto_desc_under">Дилерский центр:</span> Самара
					</div>
					<p class="auto__desc">
					<span class="auto_desc_under">Автомобиль:</span> Chery Tiggo 8
					</p>
					<p class="category__desc">
						Огромная благодарность менеджеру за помощь при выборе авто, быстро оформили сделку и рассказали все характеристики авто
					</p>
				</li>
			</ul>
		</div>
		<div class="pagination">
			<ul class="pagination_list">
				<li class="pagination_list_item p-fist"><a class="pagination__number" href="../index.php">1</a></li>
				<li class="pagination_list_item p-second"><a class="pagination__number" href="../otzyvy-vtoraya/index.php">2</a></li>
				<li class="pagination_list_item p-three"><a class="pagination__number" href="../otzyvy-tretya/index.php">3</a></li>
				<li class="pagination_list_item p-four"><a class="pagination__number" href="../otzyvy-chetvertaya/index.php">4</a></li>
				<li class="pagination_list_item p-five"><a class="pagination__number" href="../otzyvy-pyataya/index.php">5</a></li>
				<!-- <li class="pagination_list_item p-six"><a href="../otzyvy-shestaya/index.php">6</a></li>
                <li class="pagination_list_item p-seven"><a href="../otzyvy-sedmaya/index.php">7</a></li> -->
			</ul>
		</div>
	</div>
	<div class="background">
		<div class="container_site">
			<div class="flex__cont">
				<div class="middle">
					<ul class="content_li">
						<li class="title__content__item">Удовлетворённость клиентов - наш главный приоритет! И это не просто слова. Каждый день мы получаем положительные отзывы о нашей работе от покупателей и клиентов сервиса.</li>
						<li class="title__content__item">Станьте же одним из них! Запоните заявку на обратный звонок, специалист нашей компании ответит на все Ваши вопросы.</li>
					</ul>
				</div>
				<section class="form_traid-in">
					<h2 class="sub_title" id="calculator">
						Заявка на обратный звонок </h2>
					<form class="form" enctype="multipart/form-data" action="/ajax/feedback.php" method="POST">
						<div class="field">
							<label for="calc__name" class="label__traid-in">Имя*</label> <input type="text" id="calc__name" name="name" class="input__traid-in" data-validate-field="name__traid" placeholder="Ваше имя">
						</div>
						<div class="field__wrapper">
							<div class="field">
								<label for="calc__phone" class="label__traid-in">Телефон*</label> <input type="tel" id="calc__phone" name="phone" class="input__traid-in" data-validate-field="tel__traid" placeholder="Ваш телефон">
							</div>
							<div class="field">
								<label for="drive__calendar" class="label__traid">Выберите день*</label> <input type="date" id="drive__datapicker" name="datepicker" class="input__traid-in _req" placeholder="Нажмите">
							</div>
						</div>
						<div class="checkbox_popup">
							Нажимая кнопку «Отправить», вы даете Согласие на обработку персональных данных
						</div>
						<input type="hidden" name="url" value="<?= $_SERVER['REQUEST_URI'] ?>"> <input type="hidden" name="SUBJECT" value=" Заявка на Обратный звонок из Отзывов*">
						<div class="popup__btn__flex">
							<button class="btn credit__btn open_thanks_js" type="submit">Отправить</button>
						</div>
					</form>
				</section>
			</div>
		</div>
	</div>
</main>
<section class="map">
	<div class="container">
		<div class="contacts_block">
			<div class="title">
				Дилерские центры «АсАвто»
			</div>
			<ul class="list_unstyled">
				<li class="visible" data-coordinates="{&quot;lat&quot;: 53.263619542243866, &quot;lng&quot;: 50.26215135698404}" data-center="{&quot;lat&quot;: 53.263619542243866, &quot;lng&quot;: 50.25}" data-zoom="14">
					<div class="name">
						АсАвто на Алма-Атинской
					</div>
					<div class="items">
						<div class="item addr">
							г. Самара, Алма-Атинская, 87
						</div>
						<div class="item tel">
							<a href="tel:+7(8846)215-00-07">8 (846) 215 00 07</a>
						</div>
						<div class="item clock">
							Автосалон: 9:00 - 21:00
						</div>
						<div class="item clock">
							СТО: 8:00 - 20:00
						</div>
						<div class="item site">
							<a href="email:asavtomotors.ru@yandex.ru">asavtomotors.ru@yandex.ru</a>
						</div>
					</div>
				</li>
				<li data-coordinates="{&quot;lat&quot;: 53.25195107740439, &quot;lng&quot;: 50.36005176768642}" data-center="{&quot;lat&quot;: 53.25195107740439, &quot;lng&quot;: 50.346}" data-zoom="14">
					<div class="name">
						АсАвто на Аэропортовском шоссе
					</div>
					<div class="items">
						<div class="item addr">
							г. Самара, Аэропортовское шоссе, 1Ж
						</div>
						<div class="item tel">
							<a href="tel:+7(8846)215-00-07">8 (846) 215 00 07</a>
						</div>
						<div class="item clock">
							Автосалон: 9:00 - 21:00
						</div>
						<div class="item clock">
							СТО: 8:00 - 20:00
						</div>
						<div class="item site">
							<a href="email:asavtomotors.ru@yandex.ru">asavtomotors.ru@yandex.ru</a>
						</div>
					</div>
				</li>
				<li data-coordinates="{&quot;lat&quot;: 53.137290, &quot;lng&quot;: 50.179647}" data-center="{&quot;lat&quot;: 53.139475, &quot;lng&quot;: 50.167675}" data-zoom="14">
					<div class="name">
						АсАвто на Южном шоссе
					</div>
					<div class="items">
						<div class="item addr">
							г. Самара, Южное шоссе 12А, строение 4
						</div>
						<div class="item tel">
							<a href="tel:+7(8846)215-00-07">8 (846) 215 00 07</a>
						</div>
						<div class="item clock">
							Автосалон: 9:00 - 20:00
						</div>
						<div class="item clock">
							СТО: 8:00 - 20:00
						</div>
						<div class="item site">
							<a href="email:asavtomotors.ru@yandex.ru">asavtomotors.ru@yandex.ru</a>
						</div>
					</div>
				</li>
				<li data-coordinates="{&quot;lat&quot;: 53.578003950614864, &quot;lng&quot;: 49.41344120138184}" data-center="{&quot;lat&quot;: 53.578003950614864, &quot;lng&quot;: 49.40}" data-zoom="14">
					<div class="name">
						АсАвто в пос.Тимофеевка (г. Тольятти)
					</div>
					<div class="items">
						<div class="item addr">
							пос. Тимофеевка (г.Тольятти), ул. Солнечная, 1А
						</div>
						<div class="item tel">
							<a href="tel:+7(8846)215-00-07">8 (846) 215 00 07</a>
						</div>
						<div class="item clock">
							Автосалон: 9:00 - 21:00
						</div>
						<div class="item clock">
							СТО: 8:00 - 20:00
						</div>
						<div class="item site">
							<a href="email:asavtomotors.ru@yandex.ru">asavtomotors.ru@yandex.ru</a>
						</div>
					</div>
				</li>
				<li data-coordinates="{&quot;lat&quot;: 51.557420, &quot;lng&quot;: 46.066993}" data-center="{&quot;lat&quot;: 51.557420, &quot;lng&quot;: 46.052}" data-zoom="14">
					<div class="name">
						ГК АсАвто, г. Саратов
					</div>
					<div class="items">
						<div class="item addr">
							г. Саратов, Соколовая гора, 4
						</div>
						<div class="item tel">
							<a href="tel:+7(8846)215-00-07">8 (846) 215 00 07</a>
						</div>
						<div class="item clock">
							Автосалон: 9:00 - 20:00
						</div>
						<div class="item clock">
							СТО: 8:00 - 20:00
						</div>
						<div class="item site">
							<a href="email:asavtomotors.ru@yandex.ru">asavtomotors.ru@yandex.ru</a>
						</div>
					</div>
				</li>
			</ul>
		</div>
	</div>
	<div id="yamap">
	</div>
</section>
<? require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/footer.php"); ?>